...
# Gunakan pbkdf2_sha256 sebagai default yang aman dari bug 72-byte passlib, dan tetap support verify bcrypt
...
async def startup_event():
    ...
    try:
        ...
        def run_recovery():
            ...
            try:
                ...
                if recovered_count > 0:
                    recovery_db.commit()
                    print(f"[OK] Background Recovery: Berhasil memulihkan {recovered_count} password.")
            except Exception as e:
                print(f"[WARN] Background Recovery Error: {e}")
            finally:
                recovery_db.close()
        ...
        for c in coa_data:
            if c["kode_akun"] in existing_map:
                ...
                if acc.nama_akun != c["nama_akun"]:
                    ...
                    print(f"[OK] Updated COA Name: {c['kode_akun']} -> {c['nama_akun']}")
            else:
                db.add(models.AkunBukuBesar(**c))
                print(f"[OK] Added NEW COA: {c['kode_akun']} - {c['nama_akun']}")
        db.commit()
        ...
        if reclass_count > 0:
            db.commit()
            print(f"[INFO] Reclassified {reclass_count} journals for account splitting.")
        ...
        if sync_count > 0:
            db.commit()
            print(f"[OK] Synchronized {sync_count} journal names for reporting consistency.")
